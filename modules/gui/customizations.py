...
class CustomizeMenu(QtWidgets.QMenu):
    def __init__(self, title, widget, width, height, parent=None):
        ...
        #self.addAction("Scale the graph", lambda: self.scale(self, widget, width, height))
        self.addAction("Change the colours of the graph", lambda: self.changeColour(window, widget))
        #self.addAction("Change the colour of the saved graph", self.changePicColour)
    ...
    def changePicColour(self):
        print(40)

    def changeColour(self, changer, widget):
        layout = QtWidgets.QVBoxLayout()
        button1 = QtWidgets.QPushButton("Change the colour of the grid", clicked=lambda: changeGridColour(widget))
        button2 = QtWidgets.QPushButton("Change the background colour of the graph", clicked=lambda: changeBGColour(widget))
        button3 = QtWidgets.QPushButton("Change the colour of the lines to be\n drawn on the graph", clicked=lambda: changeLineColour(widget))
        layout.addWidget(button1)
        layout.addWidget(button2)
        layout.addWidget(button3)
        changer.setLayout(layout)
        changer.show()
